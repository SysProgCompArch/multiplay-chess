...
typedef struct TimeoutRequest TimeoutRequest;
typedef struct ClientMessage ClientMessage;
typedef struct PingRequest PingRequest;
typedef struct EchoRequest EchoRequest;
typedef struct MatchGameRequest MatchGameRequest;
typedef struct MoveRequest MoveRequest;
typedef struct ResignRequest ResignRequest;
typedef struct ChatRequest ChatRequest;
typedef struct ServerMessage ServerMessage;
typedef struct PingResponse PingResponse;
typedef struct EchoResponse EchoResponse;
typedef struct MatchGameResponse MatchGameResponse;
typedef struct MoveResponse MoveResponse;
typedef struct MoveBroadcast MoveBroadcast;
typedef struct CheckBroadcast CheckBroadcast;
typedef struct GameEndBroadcast GameEndBroadcast;
typedef struct ChatBroadcast ChatBroadcast;
typedef struct ErrorResponse ErrorResponse;
...
#define TIMEOUT_REQUEST__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&timeout_request__descriptor) \
     }
...
typedef enum {
  ...
  CLIENT_MESSAGE__MSG_TIMEOUT = 24
    PROTOBUF_C__FORCE_ENUM_TO_BE_INT_SIZE(CLIENT_MESSAGE__MSG__CASE)
...
typedef enum {
  ...
  SERVER_MESSAGE__MSG_ERROR = 99
    PROTOBUF_C__FORCE_ENUM_TO_BE_INT_SIZE(SERVER_MESSAGE__MSG__CASE)
...
typedef void (*TimeoutRequest_Closure)
                 (const TimeoutRequest *message,
                  void *closure_data);
...
extern const ProtobufCMessageDescriptor timeout_request__descriptor;

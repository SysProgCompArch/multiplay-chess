...
int safe_wcwidth(wchar_t ucs) {
    if (ucs == 0) return 0;
    if (ucs < 32 || (ucs >= 0x7f && ucs < 0xa0)) return -1;
    if ((ucs >= 0x1100 && ucs <= 0x115F) || (ucs >= 0xAC00 && ucs <= 0xD7A3) || (ucs >= 0x2E80 && ucs <= 0xA4CF))
        return 2;
    return 1;
}
...
void draw_chat_log() {
    ...
    for (int i = chat_line_count - 1; i >= 0 && printed < (end_row - start_row + 1); i--, row--, printed++) {
        ...
        for (int j = 0; j < wcslen(chat_log[i]) && col < CHAT_OUTLINE_BOTTOM_COL; j++) {
            int ch_width = safe_wcwidth(chat_log[i][j]);
            screen[row][col] = (screenbuffer_t){chat_log[i][j], 1};
            if (ch_width == 2 && col + 1 < CHAT_OUTLINE_BOTTOM_COL)
                screen[row][col + 1] = (screenbuffer_t){L' ', 1};
            col += ch_width;
        }
    }
}
...
int main() {
    ...
    while (1) {
        ...
        if (ch == 127 || ch == KEY_BACKSPACE) {
            if (cmd_len > 0) {
                current_width -= safe_wcwidth(cmd_buf[cmd_len - 1]);
                cmd_len--;
                cmd_buf[cmd_len] = L'\0';
            }
        } else {
            int ch_width = safe_wcwidth(ch);
            if (current_width + ch_width <= max_width && iswprint(ch)) {
                cmd_buf[cmd_len++] = ch;
                cmd_buf[cmd_len] = L'\0';
        ...
        for (int i = 0; i < cmd_len; i++) {
            int ch_width = safe_wcwidth(cmd_buf[i]);
            screen[CMD_ROW][draw_col] = (screenbuffer_t){cmd_buf[i], 1};

            // 한글 등 너비 2 문자의 다음 칸도 비워줌 (잔상 방지)
            if (ch_width == 2 && draw_col + 1 <= CMD_RIGHT) {
                screen[CMD_ROW][draw_col + 1] = (screenbuffer_t){L' ', 1};
            }
            draw_col += ch_width;
        }
